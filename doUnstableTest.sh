...
python3 usg_ui.py -n ../kubernetes_delegation/nodes.yaml -s ../TestingBaseline/services.yaml  -r 5 --checkpoint-save $latestCheckpoint -o $latestRequest
cd ..
...
while [ $(expr $(date "+%s") - $startTime) -le $totalTestLength ]; do
    ...
    cd ..
    cd Optimizer
    python3 optimizer_ui.py -n ../kubernetes_delegation/nodes.yaml -r $latestRequest -s ../TestingBaseline/services.yaml -cs ../TestingBaseline/container_specs.yaml -p ../TestingBaseline/service_ports.yaml -o ../testing-autogen-serv-kubeconf.yaml --dfo ../TestResultInfo/solution-df-$dateTag.csv
    cd ..
    kubectl apply --prune -l mist-type=service -f testing-autogen-serv-kubeconf.yaml
    ...
    cd SituationGenerator
    python3 usg_ui.py --checkpoint-load $latestCheckpoint -r 5 --checkpoint-save ../TestResultInfo/usg-checkpoint-$dateTag.pkl -o ../TestResultInfo/usg-requests-$dateTag.yaml
    cd ..
